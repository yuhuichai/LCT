% this script reads the nifti images for correction and feed them to the batch job of mc_job.m
% parameters for motion correction and estimation are adjusted in mc_job.m

clear;
close all;
batchDir='/media/yuhui/LCT/Batch'; % where the mc_job.m is located
subjDir='/media/yuhui/LCT/subj_folder/';
folderList={'mt.sft' 'bold.sft'}; %  motion correction for anatomical mt and functional bold runs at the same time
...
for folderIndex=1:length(folderList)
	...
	for imageIndex=1:length(imageList)
		runIndex=runIndex+1;
		image=rest_ReadNiftiImage_chai(imageList(imageIndex).name);
		[~,~,~,vols]=size(image);
